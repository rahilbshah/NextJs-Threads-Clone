...
export async function deleteThread(id: string, path: string): Promise<void> {
  try {
    ...
    // Remove child thread ID from other parent threads
    await Thread.updateOne(
      { _id: mainThread?.parentId?.toString() }, // Find parent threads having this thread as a child
      { $pull: { children: id } }, // Remove this thread's ID from children array of parent threads
    );
    revalidatePath(path);
  } catch (error: any) {
